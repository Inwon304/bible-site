class BibleApp {
    ...
    closeSearch() {
        ...
        document.querySelector('.search-input-container').classList.remove('has-content');
    }
    ...
    performSearch(query) {
        ...
        const results = verses.filter(v =>
            v.kjv.toLowerCase().includes(query.toLowerCase())
        );

        // Update search stats
        if (results.length === 0) {
            searchStats.textContent = `No results found for "${query}"`;
            searchResults.innerHTML = `
                <div class="search-no-results">
                    ...
                    <p>Try searching for different words or check your spelling.</p>
                </div>
            `;
        } else {
            searchStats.textContent = `Found ${results.length} result${results.length === 1 ? '' : 's'} for "${query}"`;
            const resultsHtml = results.map(v => `
                <div class="search-result" data-verse="${v.verse}">
                    <div class="verse-ref">Verse ${v.verse}</div>
                    <div class="verse-text">${v.kjv}</div>
                </div>
            `).join('');
        ...
        document.querySelectorAll('.search-result').forEach(result => {
            result.onclick = () => {
                const verseNum = parseInt(result.dataset.verse);
                this.goToVerse(verseNum);
                this.closeSearch();
            };
        });
    }
